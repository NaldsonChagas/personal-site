...
This document describes the project folder structure. The `layouts/` directory at the **repository root** is for **reference only** (HTML and PNG mockups); the actual Vue application lives under `src/`. The backend Express server lives under `src/server/` and serves the frontend build from `dist/public/`.

## Directory tree

```
dist/           # Build output: compiled Express server + Vite app in public/ (generated by pnpm build)
src/
  server/
    index.ts           # Express entry: serves dist/public/, redirects other routes to /
